...
class WishlistController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // التحقق مما إذا كانت الرفاهية موجودة بالفعل
        $existingWishlist = Wishlist::where(['user_id' => $userId, 'product_id' => $productId])->first();
    
        if ($existingWishlist) {
            // إذا كانت الرفاهية موجودة بالفعل، إرجاع رسالة جديدة
            return response()->json([
                'message' => translate('This product is already added to your wishlist'),
                'is_in_wishlist' => true,
                'wishlist_id' => (integer)$existingWishlist->id
            ], 200);
        } else {
            // إذا لم تكن الرفاهية موجودة، إضافتها
            ...
            return response()->json([
                ...
                'is_in_wishlist' => true,
                'wishlist_id' => (integer)Wishlist::where(['user_id' => $userId, 'product_id' => $productId])->first()->id
            ], 201);
        }
    }

...
class HomeController extends Controller
{
    ...
    {
        // استرجاع المستخدم باستخدام معرفه
        $user = User::findOrFail($id);

        // التحقق مما إذا كان لدى المستخدم ملف ترخيص تجاري
        if ($user->business_license) {
            // بناء مسار الملف
            $filePath = $user->business_license;

            // التحقق من نظام الملفات المستخدم
            ...
            return redirect()->back()->withErrors(['No business license found for this user.']);
        }
    }
    ...
    {
        // التحقق من أن user_id و approve تم إرسالهما مع الطلب
        if (!$request->has(['user_id', 'approve_user'])) {
            return response()->json(['success' => false], 400); // إرسال خطأ إذا لم يتم إرسال البيانات المطلوبة
        }

        $user = User::findOrFail($request->user_id);
        $user->approve_user = $request->approve_user;
        $user->save();

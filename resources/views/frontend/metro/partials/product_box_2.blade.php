...
<div class="aiz-card-box h-auto bg-white hov-scale-img">
    <div class="position-relative h-140px h-md-170px object-contain overflow-hidden">
        @php
        ...
        <a href="{{ $product_url }}" class="d-block h-100">
            <img class="lazyload mx-auto object-contain has-transition" src="{{ get_image($product->thumbnail) }}"
                alt="{{ $product->getTranslation('name') }}" title="{{ $product->getTranslation('name') }}"
                onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder.jpg') }}';" loading="lazy">
        </a>
    ...
    <div class="p-2 text-left">
        ...
        <div class="fs-14 d-flex justify-content-center mt-2">
            @if ($product->auction_product == 0)
                <!-- Previous price -->
                @if (home_base_price($product) != home_discounted_base_price($product) && auth()->check())
                ...
                @if(auth()->check())
                    <div class="">
                        <span class="fw-700 text-primary" style="color: #0c0c0b !important;">{{ home_discounted_base_price($product) }}</span>
                    </div>
                @else
                    <div class="">
                        <span class="fw-700 text-primary">{{ translate('Login to see price') }}</span>
                    </div>
                @endif
            @endif
            @if ($product->auction_product == 1)

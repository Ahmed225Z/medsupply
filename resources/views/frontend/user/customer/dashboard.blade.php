...
@section('panel_content')
    ...
    <div class="row gutters-16">
                ...
                <div class="h-100"
                    ...
                    <div class="p-4 h-100 w-100 w-xl-50">
                        ...
                        @php
                            ...
                            $Total_Expenditure = App\Models\Order::where('user_id', $user->id)
                                ->where('payment_status', 'paid')
                                ->sum('grand_total');

                            $rank = App\Models\Rank::where('min', '<=', $Total_Expenditure)
                                ->where('max', '>=', $Total_Expenditure)
                                ->first();
                        @endphp
                        <p class="fs-14 fw-400 text-gray mb-1">{{ translate('Rank') }}
                            <strong>: {{ $rank ? $rank->name : translate('No Rank Available') }}</strong>
                        </p>

                        @if ($rank)
                            <img src="{{ static_asset('Web-Images/' . $rank->image) }}"
                                class="lazyload mx-auto img-fit w-25" title="{{ $rank->name }}">
                        @endif
        ...
        <div class="col mb-4">
            <div class="h-100">
                <div class="row h-100 @if (get_setting('wallet_system') != 1 && addon_is_activated('')) row-cols-md-2 @endif row-cols-1">
                    <!-- Expenditure summary -->
                    <div class="col">
                        <div class="p-4 bg-primary @if (!addon_is_activated('club_point')) h-100 @endif"
                            ...
                            <div class="d-flex align-items-center pb-4 ">
                                <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 48 48">
                                    <g id="Group_25000" data-name="Group 25000" transform="translateclub_point(-926 -614)">
                    ...
                    @php
                        ...
                        $total_points = App\Models\ClubPoint::where('user_id', $user->id)
                            ->where('convert', 0)
                            ->sum('Points');
                    @endphp
                    <div class="col">
                        <div class="p-4 bg-warning"> <!-- هنا استخدمت bg-warning بدل bg-secondary-base -->
                            <div class="d-flex align-items-center pb-4 ">
                                <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 48 48">
                                ...
                                <div class="ml-3 d-flex flex-column justify-content-between">
                                    ...
                                    <span class="fs-20 fw-700 text-white">{{ $total_points }}</span>
                                </div>
                            </div>
